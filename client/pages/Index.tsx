...
export default function Index() {
  ...
  const handleBrowseAll = async () => {
    ...
    setSearched(true); setSearchError("");
    try {
      const response = await fetch(`/api/trips/from?${new URLSearchParams({ origin, seats: "1" })}`);
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Unable to load rides.");
      setLiveTrips(payload.trips.map((trip: any) => ({ id: trip.trip_id, type: trip.kind === "return" ? "Best match" : "Available ride", typeClass: trip.kind === "return" ? "bg-wayfare-mint text-wayfare-teal" : "bg-orange-50 text-orange-700", from: trip.origin_city, to: trip.destination_city, date: new Date(trip.departure_at).toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short" }), time: new Date(trip.departure_at).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit" }), duration: "Intercity route", seats: `${trip.available_seats} seats left`, price: `₹${Number(trip.price_per_seat).toLocaleString("en-IN")}`, oldPrice: "₹2,800", saving: "Smart fare", driver: trip.driver_name, rating: String(trip.driver_rating), initials: trip.driver_name.split(" ").map((name: string) => name[0]).join("").slice(0, 2), vehicle: trip.vehicle_name, route: "Available from your city" })));
    } catch (error) { setSearchError(error instanceof Error ? error.message : "Unable to load rides."); setLiveTrips([]); }
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setLiveTrips(payload.trips.map((trip: any) => ({
        id: trip.trip_id, type: trip.kind === "return" ? "Best match" : "Direct trip",
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-wayfare-sand text-wayfare-ink">
      ...
      <BookingDialog trip={selectedTrip} onClose={() => setSelectedTrip(null)} onAuthRequired={() => setAuthOpen(true)} />
